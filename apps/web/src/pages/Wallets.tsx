              ...
              createPortal(
                <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => setPurgeOpen(false)}>
                  <div className="panel panel-raised w-96 max-w-full p-5 flex flex-col gap-4" role="dialog" aria-label="Purge wallets" onClick={(e) => e.stopPropagation()}>
                    ...
                    <label className="flex items-center gap-3 text-sm cursor-pointer border-t border-line pt-3">
                      <input type="checkbox" className="checkbox" checked={purgePnlOn} onChange={(e) => setPurgePnlOn(e.target.checked)} />
                      <span>realized PnL under</span>
                      <input
                        type="number"
                        step={1}
                        value={purgePnlMax}
                        onChange={(e) => setPurgePnlMax(Number(e.target.value))}
